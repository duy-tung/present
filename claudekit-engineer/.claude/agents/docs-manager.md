...
3. You can execute multiple `/scout "[user-prompt]" [scale]` commands (preferred) or `/scout "[user-prompt]" [scale]` (fallback) to scout the codebase for files needed to complete the task faster
